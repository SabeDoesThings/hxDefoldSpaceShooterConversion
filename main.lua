...
Bullet.new = function() 
  local self = _hx_new(Bullet.prototype)
  Bullet.super(self)
  return self
end
Bullet.super = function(self) 
  __defold_support_Script.super(self);
end
Bullet.__name__ = true
Bullet.prototype = _hx_e();
Bullet.prototype.onMessage = function(self,messageId,message,sender) 
  if (messageId == __defold_PhysicsMessages.trigger_response) then 
    _G.go.delete();
    local emitterStateCb = nil;
    _G.particlefx.play("#explosion", (function() 
      local _hx_1
      if (emitterStateCb == nil) then 
      _hx_1 = nil; else 
      _hx_1 = function(_self,hash,hash1,state) 
        _G._hxdefold_self_ = _self;
        emitterStateCb(hash1, hash1, state);
        _G._hxdefold_self_ = nil;
      end; end
      return _hx_1
    end )());
  end;
end
Bullet.prototype.init = function(self) 
  local delay = 0;
  if (delay == nil) then 
    delay = 0;
  end;
  _G.go.animate(".", "position.y", _G.go.PLAYBACK_ONCE_FORWARD, 1000, _G.go.EASING_LINEAR, 2, delay, function(_self,url,property) 
    _G._hxdefold_self_ = _self;
    _G.go.delete();
    _G._hxdefold_self_ = nil;
  end);
end
Bullet.__super__ = __defold_support_Script
setmetatable(Bullet.prototype,{__index=__defold_support_Script.prototype})

Enemy.new = function() 
  local self = _hx_new(Enemy.prototype)
  Enemy.super(self)
  return self
end
Enemy.super = function(self) 
  __defold_support_Script.super(self);
end
Enemy.__name__ = true
Enemy.prototype = _hx_e();
Enemy.prototype.onMessage = function(self,messageId,message,sender) 
  if (messageId == __defold_PhysicsMessages.trigger_response) then 
    local playProperties = nil;
    local completeFunction = nil;
    _G.sound.play("#boom", playProperties, (function() 
      local _hx_1
      if (completeFunction == nil) then 
      _hx_1 = nil; else 
      _hx_1 = function(_self,messageId,message,sender) 
        _G._hxdefold_self_ = _self;
    ...
    _G.msg.post("interface#main", Enemy.hello);
    local playProperties = nil;
    if (playProperties == nil) then 
      _G.sprite.play_flipbook("#sprite", "enemy_death", function(_self,messageId,message,sender) 
        _G._hxdefold_self_ = _self;
        _G.go.delete();
        _G._hxdefold_self_ = nil;
      end);
...
Enemy.__super__ = __defold_support_Script
setmetatable(Enemy.prototype,{__index=__defold_support_Script.prototype})
...
Main.prototype.onMessage = function(self,messageId,message,sender) 
  Main.score = Main.score + 1;
  _G.gui.set_text(_G.gui.get_node("score"), Std.string(Main.score));
  __haxe_Log.trace("Hello", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true,customParams=true},fileName="src/Main.hx",lineNumber=12,className="Main",methodName="onMessage",customParams=_hx_tab_array({[0]=message}, 1)}));
end
Main.__super__ = __defold_support_GuiScript
setmetatable(Main.prototype,{__index=__defold_support_GuiScript.prototype})
...
Player.prototype.onInput = function(self,actionId,action) 
  if (actionId == _G.hash("left")) then 
    local pos = _G.go.get_position();
    local pos1 = pos;
    pos1.x = pos1.x - 10;
    _G.go.set_position(pos);
  else
    if (actionId == _G.hash("right")) then 
      local pos = _G.go.get_position();
      local pos1 = pos;
      pos1.x = pos1.x + 10;
      _G.go.set_position(pos);
...
Player.prototype.init = function(self) 
  _G.msg.post(".", __defold_GoMessages.acquire_input_focus);
  local playProperties = nil;
  local completeFunction = nil;
  _G.sound.play("#background", playProperties, (function() 
    local _hx_1
    if (completeFunction == nil) then 
    _hx_1 = nil; else 
    _hx_1 = function(_self,messageId,message,sender) 
  ...
  local callback = function(_,_1) 
    _G.factory.create("#factory");
    local playProperties = nil;
    local completeFunction = nil;
    _G.sound.play("#shot", playProperties, (function() 
      local _hx_2
      if (completeFunction == nil) then 
      _hx_2 = nil; else 
      _hx_2 = function(_self,messageId,message,sender) 
        _G._hxdefold_self_ = _self;
        completeFunction(messageId, message, sender);
        _G._hxdefold_self_ = nil;
      end; end
  ...
  _G.timer.delay(1, true, function(_self,handle,timeElapsed) 
    _G._hxdefold_self_ = _self;
...
__defold_support_Init.init = function(exports) 
  local script = Enemy.new();
  exports.Enemy_onMessage = function(messageId,message,sender) 
    script:onMessage(messageId, message, sender);
  end;
  local script = Main.new();
  exports.Main_onMessage = function(messageId,message,sender) 
    script:onMessage(messageId, message, sender);
  end;
  local script = Player.new();
  exports.Player_onInput = function(actionId,action) 
    do return script:onInput(actionId, action) end;
  end;
  exports.Player_init = function() 
    script:init();
  end;
  local script = Bullet.new();
  exports.Bullet_onMessage = function(messageId,message,sender) 
    script:onMessage(messageId, message, sender);
  end;
  exports.Bullet_init = function() 
    script:init();
  end;
  local defoldVersion = sys.get_engine_info().version;
  local supportedDefoldVersion = "1.6.4";
  if (defoldVersion ~= supportedDefoldVersion) then 
    _G.print(Std.string(Std.string(Std.string(Std.string("WARNING: the installed hxdefold version supports Defold ") .. Std.string(supportedDefoldVersion)) .. Std.string(", but you are running ")) .. Std.string(defoldVersion)));
  end;
end
...
local _hx_static_init = function()
  ...
  __defold_support_Init.init(_hxdefold_);Enemy.hello = _G.hash("hello");
